...
export function PuppyMaze({ onComplete, onClose, progress: baseProgress }: { onComplete: (s: number) => void; onClose: () => void; progress: number }) {
  ...
  const progress = phase === "success" ? 100 : Math.min(85, 10 + attemptsRef.current * 18 + (selected ? 5 : 0))

  return (
    <GameShell title="Lost Puppy Maze" subtitle="Help puppy get home!" progress={progress} onClose={onClose}>
      <div className="card p-5 sm:p-6 bg-card rounded-3xl border-2 border-border shadow-sm">
        <div className="flex items-start justify-between gap-3">
          <MascotMessage mood={phase === "success" ? "cheer" : phase === "blocked" ? "thinking" : "happy"} message={phase === "success" ? "Yay! Puppy made it home!" : phase === "blocked" ? "No way through. Move more rocks!" : selected ? "Tap an empty cell to move the rock there." : "Tap a rock to pick it up. Double-tap to crush it."} />
          <div className="rounded-2xl border-2 border-border bg-muted px-3 py-2 text-center">
            <p className="text-[10px] font-extrabold uppercase tracking-widest text-muted-foreground">Crushes left</p>
            <p className="font-extrabold text-xl text-primary">{removalsLeft}</p>
          </div>
        </div>

        <div className="relative mx-auto mt-5 select-none rounded-3xl border-2 border-border p-2 bg-muted/60" style={{ maxWidth: 560 }}>
          <div className="grid touch-none gap-1.5" style={{ gridTemplateColumns: `repeat(${COLS}, 1fr)` }}>
            {Array.from({ length: ROWS * COLS }).map((_, i) => {
              ...
              return (
                <button key={i} onClick={() => onCellClick(r, c)} className={`relative aspect-square rounded-xl border-2 transition-all ${isSel ? "border-primary ring-4 ring-primary/30" : isRock ? "border-amber-900 bg-amber-800" : isStart ? "border-accent-shadow bg-accent/60" : isGoal ? "border-success bg-success/20" : isPath ? "border-primary/40 bg-primary/20" : "border-border bg-background hover:bg-muted"}`}>
                  ...
                  {isPup && !isGoal && !isStart && <motion.span layoutId="puppy" className="absolute inset-0 grid place-items-center text-2xl drop-shadow">🐶</motion.span>}
                  {isPup && (isGoal || isStart) && <motion.span layoutId="puppy" className="absolute -top-2 right-0 text-2xl drop-shadow">🐶</motion.span>}
                </button>
              )
            })}
          </div>
        ...
        <div className="mt-5 flex flex-wrap items-center gap-3">
          <button onClick={go} disabled={phase !== "plan"} className="h-14 px-6 flex items-center justify-center gap-2 rounded-2xl bg-primary text-primary-foreground font-extrabold text-lg transition-transform active:scale-95 shadow-[0_4px_0_0_var(--color-primary-shadow)] border-2 border-primary-shadow disabled:opacity-50">
            <Play className="h-5 w-5" /> Send puppy
          </button>
          <button onClick={reset} disabled={phase === "walking"} className="h-14 px-6 flex items-center justify-center gap-2 rounded-2xl bg-transparent border-2 border-border text-foreground font-extrabold text-lg transition-transform active:scale-95 hover:bg-muted disabled:opacity-50">
