  ...
  return (
    ...
    >
      <div className="flex flex-col items-center gap-8 w-full max-w-xl">
        ...
        {/* The Sequence Box */}
        <div className="w-full flex gap-4 min-h-[120px] p-4 bg-[#202f36] rounded-3xl border-4 border-[#37464f] items-center justify-center">
          {sequence.length === 0 && (
            <span className="text-[#52656d] font-bold text-lg">Tap items below to build sequence</span>
          )}
          {sequence.map((item, i) => {
        ...
        <div className="flex gap-4 justify-center">
          {bank.map((item, i) => (
            <button
              ...
              className="h-24 w-24 rounded-2xl flex items-center justify-center text-5xl bg-[#37464f] border-[#202f36] border-b-4 hover:bg-[#4a5f6b] hover:translate-y-1 transition-all"
            >
              {item}
            </button>
          ))}
