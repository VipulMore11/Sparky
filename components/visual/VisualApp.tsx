...
import { saveVisualMetrics } from "@/lib/visual-store"
...
export function VisualApp({ levelIndex, onComplete, onClose }: VisualLearningLessonProps) {
  ...
  const handleNextLevel = () => {
    ...
    if ((levelIndex + 1) % 5 === 0) {
      ...
      saveVisualMetrics(levelIndex, { completed: true, score: 100 })
  ...
  if (!currentLevel) {
    return (
      <div className="fixed inset-0 z-50 bg-background text-foreground flex flex-col items-center justify-center">
        ...
        <button onClick={onClose} className="px-6 py-2 bg-primary text-primary-foreground rounded-xl font-bold">Back</button>
      </div>
    )
  }
